...
class DhtThermometer(MeasurementSource):
    def __init__(self, device_unique_name, gpio_pin):
        MeasurementSource.__init__(self, device_unique_name)
    ...
    def get_measurement(self, reading_function=None, *args, **kwargs):
        ...
        df = pd.DataFrame(
            columns=['timestamp', 'device_name', 'measurement_type', 'measurement_unit', 'measurement_value']
        )

        df.loc[0] = [
            timestamp,
            self.device_unique_name,
            self.measurement_types[0],
            self.measurement_units[0],
            measurements[0]
            ]
        ...
        df.loc[1] = [
            timestamp,
            self.device_unique_name,
            self.measurement_types[1],
            self.measurement_units[1],
            measurements[1]
            ]
